...
# AD/Azure Security Assessment - Standalone Launcher
# Can be run from any directory on domain-connected machines

set -e

# Get the directory where this script is located
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
ASSESSMENT_ROOT="$( cd "$SCRIPT_DIR/.." && pwd )"
...
DEFAULT_DATA_DIR="$ASSESSMENT_ROOT/data"
DEFAULT_OUTPUT_DIR="$ASSESSMENT_ROOT/reports"

# Banner
echo -e "${BLUE}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  AD/Azure Security Assessment - Standalone            ║${NC}"
echo -e "${BLUE}╔════════════════════════════════════════════════════════╗${NC}"
echo ""
echo "Running from: $ASSESSMENT_ROOT"
echo ""
...
if ! command -v bun &> /dev/null; then
    ...
    echo ""
    echo "Or on Windows with WSL/Git Bash, install Bun then run this script."
    exit 1
fi
...
if [ ! -f "$ASSESSMENT_ROOT/Tools/RunFullAssessment.ts" ]; then
    echo -e "${RED}✗ Assessment tools not found${NC}"
    echo "Expected location: $ASSESSMENT_ROOT/Tools/RunFullAssessment.ts"
    echo ""
...
echo ""
echo -e "${BLUE}Scanning for data sources in $DATA_DIR...${NC}"
...
PINGCASTLE_FILE=$(find "$DATA_DIR" -maxdepth 1 -name "ad_hc_*.xml" -o -name "*pingcastle*.xml" 2>/dev/null | head -n 1)
if [ -n "$PINGCASTLE_FILE" ]; then
    echo -e "${GREEN}✓ Found: PingCastle report ($(basename $PINGCASTLE_FILE))${NC}"
...
if [ ${#DATA_SOURCES[@]} -eq 0 ]; then
    ...
    echo "  - $ASSESSMENT_ROOT/Scripts/Export-ADData.ps1 (PowerShell on Windows)"
    echo "  - $ASSESSMENT_ROOT/Scripts/Export-AzureADData.ps1 (PowerShell on Windows)"
    exit 1
fi
...
eval "bun run \"$ASSESSMENT_ROOT/Tools/RunFullAssessment.ts\" $CMD_ARGS"
...
if [ $? -eq 0 ]; then
    ...
    if [ -f "$OUTPUT_DIR/executive-report.html" ]; then
        ...
        if [ "$OPEN_BROWSER" = "y" ]; then
            if command -v xdg-open &> /dev/null; then
                ...
                start "$OUTPUT_DIR/executive-report.html"
            else
    ...
    echo ""
    echo "Next steps:"
    echo "  - Review executive-report.html for high-level findings"
    echo "  - Check individual report-*.txt files for details"
    echo "  - See $ASSESSMENT_ROOT/Findings/ for remediation guides"
